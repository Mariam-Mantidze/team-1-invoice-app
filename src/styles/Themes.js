export const lightTheme = {
  backgroundColor: "rgba(248, 248, 251, 1)",
  formsBackground: "rgba(255, 255, 255, 1)",
  textColor: "rgba(12, 14, 22, 1)",
  labelColor: "rgba(126, 136, 195, 1)",
  inputBackground: "rgba(255, 255, 255, 1)",
  textFieldBorder: "1px solid rgba(223, 227, 250, 1)",
  addButtonAndInputBackground: {
    inactive: "rgba(249, 250, 254, 1)",
    active: "rgba(223, 227, 250, 1)",
  },
  saveDraftButtonBackground: {
    inactive: "rgba(55, 59, 83, 1)",
    active: "rgba(30, 33, 57, 1)",
  },
  saveDraftColor: {
    inactive: "rgba(136, 142, 176, 1)",
    active: "rgba(223, 227, 250, 1)",
  },
  textFieldHover: "1px solid rgba(124, 93, 250, 1)",
  shadow: "0px 10px 20px 0px rgba(72, 84, 159, 0.25)",
  totalColor: {
    inactive: "rgba(136, 142, 176, 1)",
    active: "rgba(136, 142, 176, 1)",
  },
  cancelButtonBacgkround: "rgba(249, 250, 254, 1)",
  cancelButtonColor: "rgba(126, 136, 195, 1)",

  discardButton: {
    inactive: "rgba(249, 250, 254, 1)",
    active: "rgba(255, 255, 255, 1)",
  },
};

export const darkTheme = {
  backgroundColor: "rgba(20, 22, 37, 1)",
  formsBackground: "rgba(20, 22, 37, 1)",
  textColor: "rgba(255, 255, 255, 1)",
  labelColor: "rgba(136, 142, 176, 1)",
  inputBackground: "rgba(30, 33, 57, 1)",
  textFieldBorder: "1px solid rgba(37, 41, 69, 1)",
  addButtonAndInputBackground: {
    inactive: "rgba(37, 41, 69, 1)",
    active: "rgba(37, 41, 69, 1)",
  },
  saveDraftButtonBackground: {
    inactive: "rgba(55, 59, 83, 1)",
    active: "rgba(30, 30, 40, 1)",
  },
  saveDraftColor: {
    inactive: "rgba(223, 227, 250, 1)",
    active: "rgba(223, 227, 250, 1)",
  },
  textFieldHover: "1px solid rgba(37, 41, 69, 1)",
  shadow: "",
  totalColor: {
    active: "rgba(255, 255, 255, 1)",
    inactive: "rgba(136, 142, 176, 1)",
  },
  cancelButtonBackground: "rgba(37, 41, 69, 1)",
  cancelButtonColor: "rgba(223, 227, 250, 1)",
  discardButton: {
    inactive: "rgba(37, 41, 69, 1)",
    active: "rgba(249, 250, 254, 1)",
  },
};
